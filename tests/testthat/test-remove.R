...
test_that("remove_rare works with matrix input", {
  ...
  test_mat <- matrix(
    c(
      ...
      NA,
      NA,
      15, # V3: 3 missing (60%) - should be removed with prop = 0.5
      16,
      17,
      18,
      19,
...
test_that("remove_rare works with matrix input and by parameter", {
  ...
  test_mat <- matrix(
    c(
      ...
      3,
      4,
      5, # V1: 1 missing in group A, 0 missing in group B
      NA,
      NA,
      8,
      9,
      10, # V2: 2 missing in group A, 0 missing in group B
      11,
      12,
      NA,
      NA,
      15 # V3: 0 missing in group A, 2 missing in group B
    ),
    nrow = 3,
    ncol = 5,
    byrow = TRUE
...
test_that("remove_low_var grouping works for by types: factor / character / numeric", {
  mat <- matrix(
    c(
      ...
      5,
      5,
      5, # V1: Both groups low variance -> remove regardless of by type (strict=TRUE)
      10,
      10,
      10,
      1,
      5,
      9, # V2: Group A low variance, Group B high variance -> remove in strict=TRUE
      1,
      2,
      3,
      4,
      5,
      6 # V3: Both groups high variance -> keep
    ),
    nrow = 3,
    byrow = TRUE
  )
...
test_that("remove_low_cv works with grouping: strict = FALSE", {
  mat <- matrix(
    c(
      ...
      5,
      5,
      5, # V1: zero CV in both groups -> remove
      10,
      10,
      10,
      1,
      5,
      9, # V2: zero CV in group A only -> keep when strict = FALSE
      1,
      2,
      3,
      4,
      5,
      6, # V3: non-zero CV in both groups -> keep
      0,
      10,
      20,
      30,
      40,
      50 # V4: non-zero CV in both groups -> keep
    ),
    nrow = 4,
    byrow = TRUE
  )
...
test_that("remove_low_cv handles rows with some NAs", {
  mat <- matrix(
    c(
      1,
      1,
      NA, # V1: zero CV across observed values -> remove
      1,
      2,
      NA, # V2: non-zero CV -> keep
      3,
      4,
      5 # V3: non-zero CV -> keep
    ),
    nrow = 3,
    byrow = TRUE
  )
...
test_that("remove_low_cv keeps all-NA row and remains stable", {
  mat <- matrix(
    c(
      NA,
      NA,
      NA, # V1: all NA -> CV helper returns Inf -> keep
      1,
      2,
      3 # V2: finite CV -> depends on cutoff
    ),
    nrow = 2,
    byrow = TRUE
  )
...
test_that("remove_low_cv keeps zero-mean row because CV is infinite", {
  mat <- matrix(
    c(
      -1,
      0,
      1, # V1: mean 0 -> CV = Inf -> keep
      10,
      12,
      14 # V2: finite CV -> may be filtered
    ),
    nrow = 2,
    byrow = TRUE
  )
...
test_that("remove_low_cv removes low CV row even with negative mean", {
  mat <- matrix(
    c(
      -10,
      -11,
      -9, # V1: CV ~ 0.1 -> remove with cutoff 0.2
      10,
      14,
      6 # V2: higher CV -> keep
    ),
