...
test_that("detect_batch_effect uses custom column names", {
  ...
  suppressMessages(
    p_values <- detect_batch_effect(exp, batch = "my_batch", group = "my_group")
  )
...
test_that("detect_batch_effect correctly identifies batch effects - mathematical validation", {
  ...
  expect_true(
    mean(vars_with_batch_effect, na.rm = TRUE) <
...
test_that("detect_batch_effect with group covariate - mathematical validation", {
  ...
  for (i in 1:n_vars) {
    for (j in 1:n_samples) {
      ...
      expr_mat[i, j] <- rnorm(
        1,
        mean = 10 + batch_effect + group_effect,
        sd = 0.5
      )
    }
  }
  ...
  suppressMessages(
    p_values_with_group <- detect_batch_effect(exp, group = "group")
  )
...
test_that("correct_batch_effect effectively removes batch effects - mathematical validation", {
  ...
  expect_true(
    significant_after < significant_before * 0.5,
...
test_that("correct_batch_effect preserves group differences - mathematical validation", {
  ...
  sample_info <- tibble::tibble(
    sample = paste0("S", 1:n_samples),
    batch = rep(
      paste0("B", 1:n_batches),
      each = n_samples_per_group_per_batch * 2
    ),
    group = rep(
      rep(c("Ctrl", "Treat"), each = n_samples_per_group_per_batch),
      n_batches
    )
  )
  ...
  expect_true(
    abs(mean_group_diff_after - true_group_effect) < 2,
...
test_that("batch correction handles direct vector inputs for experiments", {
  ...
  p_values2 <- suppressMessages(detect_batch_effect(
    exp,
    batch = batch_vector,
    group = group_vector
  ))
...
test_that("batch correction handles NULL batch parameter", {
  ...
  expect_error(
    detect_batch_effect(exp, batch = NULL),
    "Batch information is required"
  )
  expect_error(
    correct_batch_effect(exp, batch = NULL),
    "Batch information is required"
  )
})
