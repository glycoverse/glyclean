...
**Important Note:** This package works with `glyexp::experiment()` objects.
...
exp <- read_pglyco3_pglycoquant(
  here::here("vignettes", "glycopeptides.list"),
  sample_info = here::here("vignettes", "sample_info.csv"),
  glycan_type = "N"
)
exp
```
...
maybe not furlongs).
Normalization does the same thing for your glycomics data,
...
**Pro Tip:** Notice the `by` parameter in many functions? 
This allows stratified normalization within groups – 
...
| `impute_knn()` | K-nearest neighbors | Local similarity patterns |
...
you're ready to dive into the exciting world of glycomics analysis:
