...
**Important Note:** This package is designed for `glyexp::GlycomicSE` and
`glyexp::GlycoproteomicSE` objects. Preprocessing functions return the same
subclass they receive.
...
We'll use the bundled `glyexp::real_experiment` dataset, which is a
...
**Pro Tip:** 
Wrap protein expression data in a `SummarizedExperiment` object before using
...
These functions work seamlessly with `glyexp::GlycomicSE` and
`glyexp::GlycoproteomicSE` objects and provide consistent, high-quality
