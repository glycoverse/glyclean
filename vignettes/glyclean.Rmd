...
**Important Note:** This package natively supports `glyexp::GlycomicSE` and
`glyexp::GlycoproteomicSE` objects, while retaining support for legacy
`glyexp::experiment()` objects.
If you're new to these data structures,
...
We'll convert the example dataset to a native `glyexp::GlycoproteomicSE` object.

```{r}
exp <- as_glycoproteomic_se(real_experiment)
SummarizedExperiment::colData(exp)$batch <- factor(rep(c("A", "B", "C"), 4))
exp
```
...
**Pro Tip:** 
Wrap protein expression data in a supported glyco SE or legacy experiment
container before using `glyclean` preprocessing functions. Consistency is key!
...
These functions accept `glyexp::GlycomicSE`, `glyexp::GlycoproteomicSE`, and
legacy `glyexp::experiment()` objects and provide consistent, high-quality
