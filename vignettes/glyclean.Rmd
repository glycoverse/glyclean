...
`glyexp::GlycomicSE` and `glyexp::GlycoproteomicSE` objects are also supported,
and preprocessing functions return the same subclass they receive.
...
**Pro Tip:** 
Wrap protein expression data in a `glyexp::experiment()` object before using
`glyclean` preprocessing functions. Consistency is key!
...
These functions are designed to work seamlessly with `glyexp::experiment()` objects.
They also accept `glyexp::GlycomicSE` and `glyexp::GlycoproteomicSE` objects and
provide consistent, high-quality visualizations using `ggplot2`.
